...
$result = mysqli_query($koneksi, "
    SELECT p.*, 
        (
            SELECT IFNULL(SUM(penjualan),0) FROM tbl_setoran_cirebon
            WHERE tgl_setoran LIKE '$bulan%'
        ) AS total_penjualan
    FROM tbl_pengeluaran_cirebon p
...
$result_1 = mysqli_query($koneksi, "
    SELECT s.*, 
        ...
        (
            SELECT IFNULL(SUM(jml_harga),0) FROM tbl_beli_detail_cirebon
            WHERE tgl_beli LIKE '$bulan%'
        ) AS total_pembelian,
        (
            SELECT IFNULL(SUM(invoice),0) FROM tbl_pengeluaran_cirebon
            WHERE DATE_FORMAT(tgl_pengeluaran, '%Y-%m-%d') = DATE_FORMAT(s.tgl_setoran, '%Y-%m-%d')
        ) AS total_invoice
    FROM tbl_setoran_cirebon s
...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Laporan Margin Area Cirebon</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right"><li class="breadcrumb-item"><a href="<?= $main_url ?>setoran/form-setoran_cirebon.php">Setoran</a></li>
    ...
    <section class="content">
                ...
                <div class="card-body table-responsive p-3">
                    ...
                    <table class="table table-hover text-nowrap mt-2 text-center" id="tabel-penjualan">
                        ...
                        if ($result && mysqli_num_rows($result) > 0) {
                            ...
                            $result_first = mysqli_query($koneksi, "SELECT IFNULL(SUM(penjualan),0) AS total_penjualan FROM tbl_setoran_cirebon WHERE tgl_setoran LIKE '$bulan%'");
                            ...
                            $q = mysqli_query($koneksi, "SELECT IFNULL(SUM(penjualan),0) AS total_penjualan FROM tbl_setoran_cirebon WHERE tgl_setoran LIKE '$bulan%'");
        ...
        <form method="POST" action="<?= $main_url ?>laporanMargin/cirebon/rekap_margin_excel.php">
