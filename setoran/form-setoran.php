...
if (empty($_GET['tanggal_dari'])) {
    $query = "SELECT SUM(jml_harga) AS total_bayar FROM tbl_beli_detail WHERE tgl_beli = '$tanggal_hari_ini'";
    $tanggal = $tanggal_hari_ini;
} elseif (empty($_GET['tanggal_sampai'])) {
    $tanggal_dari = $_GET['tanggal_dari'];
    $query = "SELECT SUM(jml_harga) AS total_bayar FROM tbl_beli_detail WHERE tgl_beli BETWEEN '$tanggal_dari' AND '$tanggal_hari_ini'";
    $tanggal = "$tanggal_dari s/d $tanggal_hari_ini";
} else {
    $tanggal_dari = $_GET['tanggal_dari'];
    $tanggal_sampai = $_GET['tanggal_sampai'];
    $query = "SELECT SUM(jml_harga) AS total_bayar FROM tbl_beli_detail WHERE tgl_beli BETWEEN '$tanggal_dari' AND '$tanggal_sampai'";
    $tanggal = "$tanggal_dari s/d $tanggal_sampai";
}

$result = mysqli_query($koneksi, $query);
$data = mysqli_fetch_assoc($result);
$totalBayar = $data['total_bayar'] ?? 0;
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="pengeluaran" class="form-control" id="pengeluaran" value="<?=  $totalBayar ?>" readonly>
                            <small class="text-muted">Rp <?= number_format($totalBayar, 0, ',', '.') ?></small>
