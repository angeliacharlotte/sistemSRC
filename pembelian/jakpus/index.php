...
} elseif ($_SESSION["level"] != '9' && $_SESSION["level"] != '1') {
...
if($msg == 'deleted'){
    ...
    $qty = $_GET['qty'];
    $tgl = $_GET['tgl'];
    delete_jakpus($idbrg, $idbeli, $qty);
    echo "<script>
...
if ($kode) {
    $selectBrg = getData("SELECT * FROM tbl_barang_jakpus WHERE id_barang = '$kode'")[0];
}

if (isset($_POST['addbrg'])){
    $tgl = $_POST['tglNota'];
    if (insert_jakpus($_POST)){
        echo "<script>
                document.location = '?tgl=$tgl';
        </script>";
    }
...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Pembelian Barang Area Jakarta Pusat</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
                            ...
                            <div class="form-group row mb-2">
                                ...
                                <div class="col-sm-10">
                                    <select name="kodeBrg" id="kodeBrg" class="form-control">
                                        ...
                                        <?php 
                                        $barang = getData("SELECT *  FROM tbl_barang_jakpus");
                    ...
                    <div class="col-lg-6">
                        <div class="card card-outline card-danger pt-3 px-3 pb-2">
                            ...
                            <h1 class="font-weight-bold text-right" style="font-size: 40pt;">
                                <input type="hidden" name="total" value="<?= totalBeliJakpus($noBeli) ?>">
                                <?= number_format(totalBeliJakpus($noBeli), 0,',','.') ?>
                        </h1>
                        </div>
                    </div>
                </div>
                ...
                <div class="card card-outline card-success table-responsive px-2">
                    <table class="table table-sm table-hover text-nowrap">
                        ...
                        <tbody>
                            <?php
                            $no = 1;
                            $brgDetail = getData("SELECT * FROM tbl_beli_detail_jakpus WHERE no_beli ='$noBeli'");
