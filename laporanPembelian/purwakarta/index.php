...
if(empty($_GET['tanggal_dari'])) {  
  ...
  $result = mysqli_query($koneksi,"SELECT tbl_beli_detail.*, tbl_beli_head.suplier, tbl_beli_head.keterangan,  tbl_beli_head.total
        FROM tbl_beli_detail JOIN tbl_beli_head ON tbl_beli_detail.no_beli = tbl_beli_head.no_beli WHERE tbl_beli_detail.tgl_beli = '$tanggal_hari_ini' ORDER BY tbl_beli_detail.tgl_beli DESC");
}else if(empty($_GET['tanggal_sampai'])){
  $tanggal_dari = $_GET['tanggal_dari'];
  $tanggal_hari_ini = date('Y-m-d');
//   $tanggal_sampai = $_GET['tanggal_sampai'];
  $result = mysqli_query($koneksi,"SELECT tbl_beli_detail.*, tbl_beli_head.suplier, tbl_beli_head.keterangan,  tbl_beli_head.total
        FROM tbl_beli_detail JOIN tbl_beli_head ON tbl_beli_detail.no_beli = tbl_beli_head.no_beli WHERE tbl_beli_detail.tgl_beli BETWEEN '$tanggal_dari' AND '$tanggal_hari_ini' ORDER BY tbl_beli_detail.tgl_beli DESC");
}else{
    $tanggal_dari = $_GET['tanggal_dari'];
  $tanggal_sampai = $_GET['tanggal_sampai'];
  $result = mysqli_query($koneksi,"SELECT tbl_beli_detail.*, tbl_beli_head.suplier, tbl_beli_head.keterangan,  tbl_beli_head.total
...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Laporan Pembelian Area Purwakarta</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
    ...
    <section class="content">
                ...
                <div class="card-body table-responsive p-3">
                    ...
                    <table class="table table-hover text-nowrap mt-2 text-center">
                        <thead>
                            ...
                            <tr>
                                <td colspan="9">Silahkan pilih rentang tanggal</td>
                            </tr>
                            <?php } else { ?>
                        </thead>
                        <tbody>
        ...
        <form method="POST" action="<?= $main_url ?>laporanPembelian/purwakarta/rekap_pembelian_excel.php">
