...
if(empty($_GET['tanggal_dari'])) {  
  ...
  $result = mysqli_query($koneksi,"SELECT tbl_beli_detail_baksul.*, tbl_beli_head_baksul.suplier, tbl_beli_head_baksul.keterangan,  tbl_beli_head_baksul.total
        FROM tbl_beli_detail_baksul JOIN tbl_beli_head_baksul ON tbl_beli_detail_baksul.no_beli = tbl_beli_head_baksul.no_beli WHERE tbl_beli_detail_baksul.tgl_beli = '$tanggal_hari_ini' ORDER BY tbl_beli_detail_baksul.tgl_beli DESC");
}else if(empty($_GET['tanggal_sampai'])){
  $tanggal_dari = $_GET['tanggal_dari'];
  $tanggal_hari_ini = date('Y-m-d');
//   $tanggal_sampai = $_GET['tanggal_sampai'];
  $result = mysqli_query($koneksi,"SELECT tbl_beli_detail_baksul.*, tbl_beli_head_baksul.suplier, tbl_beli_head_baksul.keterangan,  tbl_beli_head_baksul.total
        FROM tbl_beli_detail_baksul JOIN tbl_beli_head_baksul ON tbl_beli_detail_baksul.no_beli = tbl_beli_head_baksul.no_beli WHERE tbl_beli_detail_baksul.tgl_beli BETWEEN '$tanggal_dari' AND '$tanggal_hari_ini' ORDER BY tbl_beli_detail_baksul.tgl_beli DESC");
}else{
    $tanggal_dari = $_GET['tanggal_dari'];
  $tanggal_sampai = $_GET['tanggal_sampai'];
  $result = mysqli_query($koneksi,"SELECT tbl_beli_detail_baksul.*, tbl_beli_head_baksul.suplier, tbl_beli_head_baksul.keterangan,  tbl_beli_head_baksul.total
...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Laporan Pembelian Area Baksul</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right"><li class="breadcrumb-item"><a href="<?= $main_url ?>pembelian/baksul">Pembelian</a></li>
    ...
    <section class="content">
        ...
        <form method="POST" action="<?= $main_url ?>laporanPembelian/baksul/rekap_pembelian_excel.php">
