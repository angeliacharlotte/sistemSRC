...
if(empty($_GET['tanggal_dari'])) {  
  ...
  $result = mysqli_query($koneksi,"SELECT tbl_beli_detail_tasik.*, tbl_beli_head_tasik.suplier, tbl_beli_head_tasik.keterangan,  tbl_beli_head_tasik.total
        FROM tbl_beli_detail_tasik JOIN tbl_beli_head_tasik ON tbl_beli_detail_tasik.no_beli = tbl_beli_head_tasik.no_beli WHERE tbl_beli_detail_tasik.tgl_beli = '$tanggal_hari_ini' ORDER BY tbl_beli_detail_tasik.tgl_beli DESC");
}else if(empty($_GET['tanggal_sampai'])){
  $tanggal_dari = $_GET['tanggal_dari'];
  $tanggal_hari_ini = date('Y-m-d');
//   $tanggal_sampai = $_GET['tanggal_sampai'];
  $result = mysqli_query($koneksi,"SELECT tbl_beli_detail_tasik.*, tbl_beli_head_tasik.suplier, tbl_beli_head_tasik.keterangan,  tbl_beli_head_tasik.total
        FROM tbl_beli_detail_tasik JOIN tbl_beli_head_tasik ON tbl_beli_detail_tasik.no_beli = tbl_beli_head_tasik.no_beli WHERE tbl_beli_detail_tasik.tgl_beli BETWEEN '$tanggal_dari' AND '$tanggal_hari_ini' ORDER BY tbl_beli_detail_tasik.tgl_beli DESC");
}else{
    $tanggal_dari = $_GET['tanggal_dari'];
  $tanggal_sampai = $_GET['tanggal_sampai'];
  $result = mysqli_query($koneksi,"SELECT tbl_beli_detail_tasik.*, tbl_beli_head_tasik.suplier, tbl_beli_head_tasik.keterangan,  tbl_beli_head_tasik.total
...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Laporan Pembelian Area Tasikmalaya</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right"><li class="breadcrumb-item"><a href="<?= $main_url ?>pembelian/tasikmalaya">Pembelian</a></li>
    ...
    <section class="content">
        ...
        <form method="POST" action="<?= $main_url ?>laporanPembelian/tasikmalaya/rekap_pembelian_excel.php">
