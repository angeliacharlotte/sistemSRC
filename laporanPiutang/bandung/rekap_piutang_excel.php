...
$result = mysqli_query($koneksi,"SELECT tbl_jual_detail_bandung.*, tbl_jual_head_bandung.customer, tbl_jual_head_bandung.keterangan,  tbl_jual_head_bandung.jml_bayar
...
$sheet->setCellValue('A1', 'REKAP PIUTANG AREA BANDUNG');
...
$row = 6;
$piutang_per_nota = [];
$transaksi_data = []; // Untuk menyimpan total qty dan harga per nota
...
while($data = mysqli_fetch_array($result)) {
    ...
    // Hitung total tagihan per nota (akumulasi per baris)
    if (!isset($transaksi_data[$no_jual])) {
        $transaksi_data[$no_jual] = [
            'total_tagihan' => 0,
            'total_bayar' => $jml_bayar, // diasumsikan sama untuk semua baris per nota
    ...
    // Tulis data ke Excel per baris barang
...
// Hitung total piutang dari transaksi yang belum lunas
$total_piutang = 0;
foreach ($transaksi_data as $nota => $trx) {
...
// Tampilkan total piutang di baris akhir
...
header('Content-Disposition: attachment;filename="rekap_piutang_bandung.xlsx"');
