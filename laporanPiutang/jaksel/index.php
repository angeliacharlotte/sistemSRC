...
if(empty($_GET['tanggal_dari'])) {  
  ...
  $result = mysqli_query($koneksi,"SELECT tbl_jual_detail_jaksel.*, tbl_jual_head_jaksel.customer, tbl_jual_head_jaksel.keterangan,  tbl_jual_head_jaksel.jml_bayar, tbl_jual_head_jaksel.total
        FROM tbl_jual_detail_jaksel JOIN tbl_jual_head_jaksel ON tbl_jual_detail_jaksel.no_jual = tbl_jual_head_jaksel.no_jual WHERE tbl_jual_detail_jaksel.tgl_jual = '$tanggal_hari_ini' ORDER BY tbl_jual_detail_jaksel.tgl_jual DESC");
}else if(empty($_GET['tanggal_sampai'])){
  $tanggal_dari = $_GET['tanggal_dari'];
  $tanggal_hari_ini = date('Y-m-d');
//   $tanggal_sampai = $_GET['tanggal_sampai'];
  $result = mysqli_query($koneksi,"SELECT tbl_jual_detail_jaksel.*, tbl_jual_head_jaksel.customer, tbl_jual_head_jaksel.keterangan,  tbl_jual_head_jaksel.jml_bayar, tbl_jual_head_jaksel.total
        FROM tbl_jual_detail_jaksel JOIN tbl_jual_head_jaksel ON tbl_jual_detail_jaksel.no_jual = tbl_jual_head_jaksel.no_jual WHERE tbl_jual_detail_jaksel.tgl_jual BETWEEN '$tanggal_dari' AND '$tanggal_hari_ini' ORDER BY tbl_jual_detail_jaksel.tgl_jual DESC");
}else{
    $tanggal_dari = $_GET['tanggal_dari'];
  $tanggal_sampai = $_GET['tanggal_sampai'];
  $result = mysqli_query($koneksi,"SELECT tbl_jual_detail_jaksel.*, tbl_jual_head_jaksel.customer, tbl_jual_head_jaksel.keterangan,  tbl_jual_head_jaksel.jml_bayar, tbl_jual_head_jaksel.total
        FROM tbl_jual_detail_jaksel JOIN tbl_jual_head_jaksel ON tbl_jual_detail_jaksel.no_jual = tbl_jual_head_jaksel.no_jual WHERE tbl_jual_detail_jaksel.tgl_jual BETWEEN '$tanggal_dari' AND '$tanggal_sampai' ORDER BY tbl_jual_detail_jaksel.tgl_jual DESC");
}
...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Laporan Penjualan Area Jakarta Selatan</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right"><li class="breadcrumb-item"><a href="<?= $main_url ?>penjualan/jaksel">Penjualan</a></li>
                            ...
                            while($brg = mysqli_fetch_array($result)) :
                             ?>
                                <tr>
                                    ...
                                    <td class="keterangan"><?= $brg['keterangan'] ?></td>
...
window.onload = hitungSisaPembayaran;
        ...
        <form method="POST" action="<?= $main_url ?>laporanPenjualan/jaksel/rekap_penjualan_excel.php">
