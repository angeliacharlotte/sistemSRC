...
if(empty($_GET['tanggal_dari'])) {  
  ...
  $result = mysqli_query($koneksi,"SELECT tbl_jual_detail.*, tbl_jual_head.customer, tbl_jual_head.keterangan,  tbl_jual_head.jml_bayar, tbl_jual_head.total
...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Laporan Piutang Area Purwakarta</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right"><li class="breadcrumb-item"><a href="<?= $main_url ?>penjualan/purwakarta">Penjualan</a></li>
                    ...
                    <table class="table table-hover text-nowrap mt-2" id="tabel-piutang">
                        ...
                        <tbody>
                            ...
                            while($brg = mysqli_fetch_array($result)) :
                             ?>
                                <tr>
                                    ...
                                    <td class="text-center tagihan"><?= number_format($brg['jml_bayar'],0,',','.' )?></td>
                            ...
                            <div style="display: flex; align-items: center; gap: 10px; margin-left: 980px; margin-right: 20px; margin-top: 20px;">
                                ...
                                <input type="text" name="grandTotal" class="form-control text-right" id="grandTotal" readonly>
                            </div>
<script>
function hitungSisaPembayaran() {
  ...
  rows.forEach(row => {
    ...
    const bayarText = row.querySelector('td:nth-child(8)')?.textContent.trim().replace(/\./g, '') || '0';
    const tagihan = row.querySelector('td:nth-child(9)')?.textContent.trim().replace(/\./g, '') || '0';
    const keterangan = row.querySelector('td:nth-child(10)')?.textContent.trim();    

    let total_tagihan = hargaText - bayarText;

    // Tampilkan total di kolom "Total"
      row.querySelector(".tagihan").textContent = total_tagihan;

      // Tambahkan ke grand total
      grandTotal += total_tagihan;
    
  });
  // Tampilkan grand total ke input text
...
// Fungsi format rupiah (tanpa Rp)
function formatRupiah(angka) {
  return angka.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
}

window.addEventListener('load', hitungSisaPembayaran);
</script>
